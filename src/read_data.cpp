...
void read_off_file_attrib(unsigned int& vertices, 
		unsigned int& facets, unsigned int& edges,
...
template <typename arr_v, typename arr_f> void read_data(arr_v& vertices_arr, 
		arr_f& faces_arr, 
		std::string file_name){
  ...
  if(buffer == "OFF"){
	...
	while(it <= (3 * vertices + 4 * facets)){
		...
		if((counter % 4) != 0){
		  ...
		  if(counter < 50) std::cerr << buffer_uint << " ";
		}
		it++; counter++;
	}
